    ...
    def __init__(self, render_mode=None):
        ...
        self.observation_space = spaces.Dict(
            {"matrix": spaces.MultiBinary((((2 + self.n_bugs) * (1 + 2 * self.n_bugs)) * 2)),
             "sample_input__output_pair": spaces.MultiDiscrete( # TODO https://gymnasium.farama.org/api/spaces/fundamental/#multidiscrete
                    shape=(self.sample_size, self.n_bugs), # 3??
                    dtype=np.int64
                ),
                # "sample_output_pairs": spaces.Discrete( #komplett weglassen oder als eigenen space?!
                #     low=0,
                #     high=1,
        ...
        #     (((2 + self.n_bugs) * (1 + 2 * self.n_bugs)) * 2))
