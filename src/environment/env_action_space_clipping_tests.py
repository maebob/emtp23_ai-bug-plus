...
from src.utils.translate_action import translate_action#, translate
from src.utils.error_to_clipping import translate_to_range
...
df = pd.read_csv("/Users/mayte/GitHub/BugPlusEngine/configs_4x+4y.csv", sep=";", header=None)
df = df.dropna(axis=0, how='all') # drop empty rows
DF = df.sample(frac=1, random_state=42069).reset_index() # shuffle rows, keep index
# IDEA:
# clip action space with reset
# then at the end of each step, change the clipped action space
# so that the agent can only choose from the clipped action space at all times.
""" 
# Port In of bug 0 is not connected to anything
# Port In of bug 0 is not connected to anything
# Port In of bug 0 is not connected to anything
        try:
...
class BugPlus(Env):
    def __init__(self, render_mode=None):
        ...
        clip = find_action_space(self)
        print("init clip: ", clip)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.episode_length = 0
        # TODO: action clipping here as welL?
        return self.state, {}
    ...
    def step(self, action_original: torch):
        ...
        if action_original not in self.action_space:
            ...
            if action_original < self.action_space.low:
                action_original = self.action_space.low
            



        action = translate_action(self.no_bugs, action_original)
        print(self.action_space, "action agent:", action_original, "translated action:", action)
        ...
        if reward <= 0 and done:
            ...
            self.load_new_config = True
        #TODO: check!
        # action space clipping after each step
        clip = find_action_space(self)
        self.action_space = spaces.Box(low=clip[0], high=clip[1], shape=(1,), dtype=np.int32) #TODO
        return self.state, reward, done, truncated, {'ep_return': self.ep_return}
